...
void Scene::createImage()
{
	...
	for (int i =  0; i < camera.width; i++)
	{
		for (int j = 0; j < camera.height; j++)
		{
			...
			{
				int res = r.intersect(tabSphere[k]);
				if (res != -1)
				{
			...
			else
			{
				...
				Vec3 posTouche = r.position + r.direction * resMin;
				...
				if (noLight)
				{
					color.rgbRed = 255 * 0.1 * tabSphere[indMin].couleur.x;
					color.rgbGreen = 255 * 0.1 * tabSphere[indMin].couleur.x;
					color.rgbBlue = 255 * 0.1 * tabSphere[indMin].couleur.x;
				}

				FreeImage_SetPixelColor(bitmap, camera.width-(i+1), j, &color);
			}
		}
	}
